...
function PlatformCard({ platform }: { platform: PlatformSlip }) {
  ...
  // Guide cards start expanded so users see the selections before relying on the code
  const [expanded, setExpanded] = useState(!codeIsReal);
  ...
  return (
    ...
    >
      ...
      {/* ── Real booking code row (live codes only) ── */}
      {codeIsReal && platform.bookingCode && (
        <div className="flex items-center gap-2 px-4 pb-3">
          <span className="flex-1 font-mono text-xl font-bold tracking-widest text-ov-text truncate">
            {platform.bookingCode}
          </span>
          <CopyBtn text={platform.bookingCode} color={accent} />
        </div>
      )}

      {/* ── Guide section (non-real codes) ── */}
      {!codeIsReal && (
        <>
          {/* Explanation — no fake code shown, just guide */}
          <div className="px-4 pb-2.5">
            {platform.selections.length > 0 ? (
              <p className="text-xs text-ov-amber/80 leading-snug">
                No direct code available. Verify the selections below, add them on{' '}
                <span className="font-semibold text-ov-amber">{platform.platformName}</span>, then
                share your betslip to get a real booking code.
              </p>
            ) : (
              <p className="text-xs text-ov-amber/80 leading-snug">
                Visit{' '}
            ...
            <div className="border-t border-ov-border/60">
              ...
              >
                <span>
                  ...
                  · {expanded ? 'collapse guide' : 'view guide'}
                </span>
                {expanded
                    ...
                    <div className="divide-y divide-ov-border/50">
                      ...
                      <div className="px-4 py-2.5 flex items-center justify-between bg-ov-elevated/30">
                        <p className="text-ov-faint text-xs">
                          Add all selections → tap Share / Book to get your real code
                        </p>
                        <a
                          href={platform.platformUrl}
                          target="_blank"
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-center text-xs text-ov-faint leading-relaxed px-4">
        <span className="text-ov-green font-semibold">Live code</span> = real, paste directly into the platform.{' '}
        <span className="text-ov-amber font-semibold">Guide only</span> = no code generated — add the selections manually on that platform to get a real booking code. Always verify odds before placing.
      </p>
    </div>
  );
}
